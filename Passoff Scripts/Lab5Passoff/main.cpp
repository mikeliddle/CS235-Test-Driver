/* Fixes by Jason Anderson:
	is_valid_path allows path to skip multiple cells if only one variable changes. FIXED.
	is_valid_path accepts paths that do not start at (0, 0, 0). FIXED.
	Invalid paths don't include "Path Given:" when printed. FIXED.
	solveMaze allows students to change the maze to be all 1's after original print. FIXED.
	main has annoying pause at the end. FIXED.
*/
#include <iostream>
#include <fstream>
#include <sstream>
...
void parse_instruction(std::string temp, std::ofstream &ofs, Pathfinder* aptr) {
	...
	if (command == "importMaze") { // command to import a new maze from a file
		...
		// This is removed because of redundancy.
